<template>
  <div class="min-h-screen bg-bg font-sans antialiased text-main selection:bg-main selection:text-bg">
    ...
    >
      ...
      <div class="h-16 flex items-center justify-between px-6 border-b border-border dark:border-white/5">
        <NuxtLink to="/app" class="flex items-center gap-3 transition-opacity hover:opacity-80" @click="isSidebarOpen = false">
          ...
          <span class="text-base font-bold tracking-tight text-main dark:text-white">Viewora</span>
        </NuxtLink>
      ...
      <!-- User Profile Area -->
      <div class="p-4 border-t border-border dark:border-white/5 bg-surface-alt/50 dark:bg-white/5">
        <div class="flex items-center gap-3 mb-4 px-2">
           <div class="w-9 h-9 rounded-full bg-surface-alt dark:bg-white/5 border border-border dark:border-white/10 flex items-center justify-center text-xs font-bold text-main dark:text-white shadow-sm">
             {{ authStore.avatarInitials }}
           </div>
           <div class="flex-1 min-w-0">
             <p class="text-sm font-bold text-main dark:text-white truncate">{{ user?.email?.split('@')[0] }}</p>
             <p class="text-[10px] font-extrabold uppercase tracking-widest text-dim truncate">{{ planStore.plan?.name || 'Free' }} Plan</p>
           </div>
        </div>

        <!-- Theme toggle -->
        <button @click="toggleTheme" class="flex items-center gap-3 w-full px-4 py-3 text-xs font-black uppercase tracking-widest text-dim hover:text-main dark:text-zinc-400 dark:hover:text-white hover:bg-main/5 dark:hover:bg-white/5 rounded-2xl transition-all mb-2 group active:scale-95">
          <!-- Sun icon (shown in dark mode) -->
          <svg v-if="isDark" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="group-hover:rotate-45 transition-transform duration-500"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>
          <!-- Moon icon (shown in light mode) -->
          <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="group-hover:-rotate-12 transition-transform duration-500"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
          <span>{{ isDark ? 'Light' : 'Dark' }} Appearance</span>
        </button>
 
        <button @click="logout" class="flex items-center gap-3 w-full px-4 py-3 text-xs font-black uppercase tracking-widest text-rose-500 hover:bg-rose-500/10 rounded-2xl transition-all active:scale-95 group">
    ...
    <div class="lg:pl-64 flex flex-col min-h-screen">
      ...
      <!-- App View -->
      <main class="flex-1 px-4 sm:px-8 md:px-12 lg:px-16 py-8 md:py-12 w-full max-w-[1600px] mx-auto overflow-x-hidden">
        <slot />
      </main>
    </div>
