  ...
  <div class="h-full flex flex-col items-center justify-center animate-in fade-in duration-300">
    ...
    <div v-if="step === 1" class="w-full max-w-[900px] mt-12 px-4 sm:px-6 flex flex-col items-center">
      <div class="text-center mb-10 w-full">
        <h1 class="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">What are you creating?</h1>
        <p class="text-base text-zinc-500 dark:text-zinc-400">Choose the type of tour to get the best setup</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 w-full max-w-[800px] mx-auto">
        <div 
          ...
          :key="option.id"
          class="flex items-center h-[160px] p-6 rounded-2xl transition-all duration-150 cursor-pointer border bg-white dark:bg-zinc-900"
          :class="[
            selectedType === option.id 
              ? 'border-zinc-900 shadow-sm ring-1 ring-zinc-900 dark:ring-zinc-100' 
              : 'border-zinc-200 dark:border-zinc-700 hover:-translate-y-0.5 hover:shadow-md hover:border-zinc-300 dark:hover:border-zinc-600'
          ]"
          @click="selectedType = option.id"
        >
          <div 
            class="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 transition-colors"
            :class="selectedType === option.id ? 'bg-zinc-900 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400'"
          >
            <!-- Property -->
          ...
          <div class="ml-5 flex-1">
            <h3 class="text-[16px] font-bold text-zinc-900 dark:text-zinc-100 leading-tight">{{ option.title }}</h3>
            <p class="text-[13px] text-zinc-500 dark:text-zinc-400 mt-1.5 leading-snug">{{ option.description }}</p>
          </div>
        </div>
      </div>
    ...
    <div v-if="step === 2" class="w-full max-w-[600px] mt-12 px-4 sm:px-6 flex flex-col items-center animate-in slide-in-from-right-4 duration-300">
      <div class="mb-8 w-full">
        <button @click="step = 1" class="text-sm font-medium text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-zinc-100 mb-8 flex items-center gap-1 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          Back
        </button>
        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-lg text-[13px] font-semibold mb-4 border border-zinc-200 dark:border-zinc-700">
          Creating: {{ selectedOptionData?.title }} {{ selectedOptionData?.emoji }}
        </div>
        <h1 class="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">Basic Setup</h1>
        <p class="text-base text-zinc-500 dark:text-zinc-400">Give your tour a name to start building.</p>
      </div>

      <div class="w-full bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-sm space-y-5">
        <div class="space-y-1.5">
          <label class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Tour Name <span class="text-red-500">*</span></label>
          <input 
            v-model="tourName" 
            type="text" 
            placeholder="e.g. 123 Main Street" 
            autofocus
            class="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-zinc-900 dark:text-zinc-100 text-base outline-none focus:bg-white dark:bg-zinc-900 focus:border-zinc-400 dark:focus:border-zinc-500 focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-600 transition-colors"
          />
        </div>
        
        <div class="space-y-1.5">
          <label class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Location <span class="text-zinc-400 font-normal">(Optional)</span></label>
          <input 
