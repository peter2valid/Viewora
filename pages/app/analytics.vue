    ...
    <div v-else-if="pending" class="space-y-6 animate-pulse">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="grid grid-cols-2 grid-rows-3 gap-3">
          <div v-for="n in 6" :key="n" class="bg-card rounded-2xl p-4">
    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="tourStats.length > 1" class="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        <button
          ...
          :class="!selectedInsightsTourId ? 'bg-main text-bg' : 'bg-surface-alt text-main border border-border dark:border-transparent'"
        >All Tours</button>
        <button
          ...
          :class="selectedInsightsTourId === tour.id ? 'bg-main text-bg' : 'bg-surface-alt text-main border border-border dark:border-transparent'"
        >{{ tour.title }}</button>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <!-- Metric Tiles (1/3) — 3×2 grid, fills chart height -->
        <div class="grid grid-cols-2 grid-rows-3 gap-3">
          <div v-for="metric in [
            ...
            { label: 'WhatsApp Clicks', value: totalWhatsappClicks, sub: 'Buyer intent', icon: 'whatsapp' },
            { label: 'Saves', value: totalSaves, sub: 'Buyer intent', icon: 'heart' },
...
const rawEngagementStats = ref<{ property_id: string; action: 'whatsapp_click' | 'save' }[]>([])
...
async function fetchStats() {
  ...
  try {
    const [statsData, engagementData] = await Promise.all([
      ...
      apiFetch<any[]>('/analytics/engagement').catch(() => []), // non-critical — tiles just show 0
    ])
    rawStats.value = statsData || []
    rawEngagementStats.value = engagementData || []
  } catch {
...
const filteredEngagementStats = computed(() => {
  if (!selectedInsightsTourId.value) return rawEngagementStats.value
  return rawEngagementStats.value.filter(s => s.property_id === selectedInsightsTourId.value)
})
const totalWhatsappClicks = computed(() => filteredEngagementStats.value.filter(s => s.action === 'whatsapp_click').length)
const totalSaves = computed(() => filteredEngagementStats.value.filter(s => s.action === 'save').length)
